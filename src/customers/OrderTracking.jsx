...
import { X, Search, PackageCheck, ClipboardList, RefreshCw, CheckCircle2 } from 'lucide-react';
        ...
        {searchedOrder && (
          <div className="space-y-6 animate-scale-up">
            <div className="bg-gray-50 border border-gray-100 rounded-xl p-4 text-xs space-y-1.5">
              ...
              <div className="flex justify-between"><span className="text-gray-400 font-medium">Destination Terminal:</span> <span className="font-bold text-gray-800 text-right max-w-[200px] truncate">{searchedOrder.deliveryAddress}</span></div>
                ...
                {statuses.map((step, idx) => {
                  ...
                  return (
                    <div key={step} className="flex flex-col items-center flex-1 relative">
                      <div className={`w-6 h-6 rounded-full flex items-center justify-center border-2 text-[10px] font-bold transition-all duration-300 ${
                        isCurrent ? 'bg-orange-600 border-orange-600 text-white shadow-md scale-110' :
